  ...
  THERMISTOR: {
    initialize: {
      value: function (opts, dataHandler) {
        ...
        var nominalResistance = opts.nominalResistance || 5000;
        var betaCoefficient = opts.betaCoefficient || 3500;
        var temperatureNominal = opts.temperatureNominal || 25;
        var reportInterval = opts.reportInterval || 1; // Use global reporting interval as default
        var reportIntervalUnitSeconds = false;

        if (reportInterval > 16383) {
          reportIntervalUnitSeconds = true;
          reportInterval = Math.round(reportInterval / 1000);
        }

        function convertTemp(analogValue) {
          var resistance = 1023 / analogValue - 1;
        ...
        this.io.analogReadCustomInterval(pin, reportInterval, reportIntervalUnitSeconds, function(data) {

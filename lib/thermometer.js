...
function analogHandler(opts, dataHandler) {
  ...
  this.io.pinMode(this.io.analogPins[pin], this.io.MODES.ANALOG);
...
var Controllers = {
  ...
  THERMISTOR: {
    initialize: {
      value: function (opts, dataHandler) {
        var pin = opts.pin;
        var powerPin = opts.powerPin;
        var seriesResistorOhms = opts.seriesResistorOhms;
        var resistorToGround = opts.resistorToGround || true;
        var nominalResistance = opts.nominalResistance || 5000; // Most common value
        var betaCoefficient = opts.betaCoefficient || 3500; // Most common value
        var temperatureNominal = opts.temperatureNominal || 25; // Most common value
        var reportInterval = opts.reportInterval; // Use global reporting interval by default
        var adcResolution = this.io.RESOLUTION.ADC;

        function convertTemp(analogValue) {
          var resistance = adcResolution / analogValue - 1;
          if (resistorToGround) {
            // Resistor is connected to ground, so we're measuring the drop on the resistor
            resistance = seriesResistorOhms * resistance;
          } else {
            // Thermistor is connected to ground, so we're measuring the drop on the thermistor
            resistance = seriesResistorOhms / resistance;
          }

          var steinhart = resistance / nominalResistance; // (R/Ro)
          steinhart = Math.log(steinhart); // ln(R/Ro)
          steinhart /= betaCoefficient; // 1/B * ln(R/Ro)
          steinhart += 1.0 / (temperatureNominal + CELSIUS_TO_KELVIN); // + (1/To)
          steinhart = 1.0 / steinhart; // Invert
          steinhart -= CELSIUS_TO_KELVIN; // convert to C
        ...
        this.io.pinMode(this.io.analogPins[pin], this.io.MODES.ANALOG);
        // if (powerPin) {
        //   this.io.pinMode(powerPin, this.io.MODES.OUTPUT);
        //   this.io.digitalWrite(powerPin, this.io.LOW);
        // }
        if (reportInterval) {
          // Fetch data at the desired interval
          setInterval(function() {
            // if (!this.enabled) return;
            // if (powerPin) {
            //   // Power up the sensor
            //   this.io.digitalWrite(powerPin, this.io.HIGH);
            // }
            // Read data
            this.io.extendedAnalogRead(pin, function(data) {
              // if (powerPin && !pendingRead) {
              //   // Power down the sensor
              //   this.io.digitalWrite(powerPin, this.io.LOW);
              // }
              dataHandler.call(this, convertTemp(data));
            }.bind(this));
          }.bind(this), reportInterval);
        } else {
          // Default: will report at the main reporting loop
          // if (powerPin) {
          //   // Power up the sensor
          //   this.io.digitalWrite(powerPin, this.io.HIGH);
          // }
          this.io.extendedAnalogRead(pin, { enableReporting: true }, function(data) {
            dataHandler.call(this, convertTemp(data));
          }.bind(this));
        }
      }
  ...
  DHT_CONFIGURABLE_FIRMATA: {
    initialize: {
      value: function(opts, dataHandler) {
        var Multi = require("./imu");
        var driver = Multi.Drivers.get(this.board, "DHT_CONFIGURABLE_FIRMATA", opts);
        driver.on("data", function(data) {
          dataHandler(data.temperature);
        });
      }
    ...
    toCelsius: {
      value: function(raw) {
        // data is received with 0.1 °C precision

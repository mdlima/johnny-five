  ...
  DHT_CONFIGURABLE_FIRMATA: {
    ...
    initialize: {
      value: function(board, opts) {
        var io = board.io;
        // Correspond to firmware variables
        var dhtPin = opts.pin || 2;
        var dhtType = opts.type || this.REGISTER.DHTSENSOR_TYPE.DHT22;
        var freq = opts.freq || 2000;
        var freq_unit_s = false;

        // When in debug mode, if pin is not an Interrput pin, emit an error
        if (opts.debug && !board.pins.isInterrupt(dhtPin)) {
          Board.Pins.Error({
            pin: dhtPin,
            type: "Interrupt",
            via: "DHT_CONFIGURABLE_FIRMATA",
          });
        }

        if (freq < 2000 && !freq_unit_s) {
          freq = 2000;
        }
        if (freq > 16383) { // biggest time in ms (14bit int)
          // use seconds as unit and rount the frequency
          freq_unit_s = true;
        ...
        // Attach the sensor
        io.dhtSensorAttach(dhtPin, dhtType, freq, freq_unit_s);
        io.on("dht-read", function(dhtValues) {
          // Update with current state
          // TODO: check for inconsistencies and handle error cases
          computed.temperature = dhtValues.tempValue;
          computed.humidity = dhtValues.humidityValue;
          this.emit("data", computed);
        }.bind(this));
      }
    },
    identifier: {
      value: function(opts) {
        return "dht_configurable_firmata_" + opts.type;
      }
    }
  },
...
var Controllers = {
  ...
  DHT_CONFIGURABLE_FIRMATA: {
    initialize: {
      value: function(opts) {
        Components.call(this, "DHT_CONFIGURABLE_FIRMATA", opts);
      }
    },
    components: {
      value: [HYGROMETER, THERMOMETER]
...
Controllers.DHT_CONFIGURABLE_FIRMATA = Controllers.DHT_CONFIGURABLE_FIRMATA;
